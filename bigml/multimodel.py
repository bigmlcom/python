        ...
        for model in self.models:
            ...
            if reuse:
                try:
                    ...
                    pass
            predictions_file = open(output_file, 'w', 0)
            for input_data in input_data_list:
                predictions_file.write(model.predict(input_data,
                                                     by_name=by_name) + "\n")
            predictions_file.close()


    def batch_votes(self, predictions_file_path):
        ...
        predictions_files = []
        for model in self.models:
            predictions_files.append(open(
                get_predictions_file_name(model.resource_id,
                predictions_file_path), "rb"))
        votes = []
        predictions = {}
        prediction = True
        while prediction:
            if predictions:
                ...
                predictions = {}
            for handler in predictions_files:
                prediction = handler.readline().rstrip()
                if not prediction:
                    break
